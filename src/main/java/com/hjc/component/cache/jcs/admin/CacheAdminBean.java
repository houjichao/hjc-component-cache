package com.hjc.component.cache.jcs.admin;
 ...
 */
public class CacheAdminBean {
	/** The logger. */
	public static final Logger log = LoggerFactory
			.getLogger(CacheAdminBean.class);

	/** The cache maanger. */
	private CompositeCacheManager cacheHub = CompositeCacheManager
			.getInstance();
	...
	public LinkedList buildElementInfo(String cacheName) throws Exception {
		...
		for (int i = 0; i < keys.length; i++) {
			...
			elementInfo.createTime = format.format(new Date(attributes
					.getCreateTime()));

			elementInfo.expiresInSeconds = (now - attributes.getCreateTime() - (attributes
					.getMaxLifeSeconds() * 1000)) / -1000;
	 ...
	 * <p>
	 * 
	 * we need a most light weight method that does not count bytes. The byte
	 * counting can really swamp a server.
	 * 
	...
	public int getByteCount(CompositeCache cache) throws Exception {
		...
		try {
			...
			log.info("Problem getting byte count.  Likley cause is a non serilizable object."
					+ e.getMessage());
		} finally {
			out.close();
		}
